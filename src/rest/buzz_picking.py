...
from src.provider import get_browser_manager, get_buzz_runner
from src.service import BrowserManager, BuzzRunner
...
async def pick_all_buzz(
        ...
        buzz_runner: BuzzRunner = Depends(get_buzz_runner)
    ):
    if len(session_urls) > 0:
        await browser_manager.open_browser(session_urls[0])
        await buzz_runner.pick_all_buzz(session_urls)
