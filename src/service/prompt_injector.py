from src.constant import global_timeout, model_search_input_selector, WAIT_PREFIX, DONE_PREFIX, \
    generation_info_button_selector, creator_tip_selector, civitai_tip_selector, no_more_buzz_triangle_svg_selector, \
    generation_button_selector, all_jobs_done_selector
...
class PromptInjector:
    ...
    async def generate_till_no_buzz(self):
        ...
        while buzz_remain is True:
            ...
            if buzz_remain:
                await self.page.locator(generation_button_selector).wait_for(timeout=120000)
                await self.page.locator(generation_button_selector).click()
                await asyncio.sleep(3)
        logger.info(DONE_PREFIX + "launch_all_generations")
        await self.page.locator(all_jobs_done_selector).wait_for(timeout=120000)
